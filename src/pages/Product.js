  ...
  return (
    <>
      <section className='pb-24'>
        ...
        <div class="m-4 mx-auto max-w-screen-lg rounded-md border border-gray-100 text-neutral-200 shadow-md">
          <div class="relative flex h-full flex-col  md:flex-row">
            <div class="relative p-8 md:w-4/6">
              <div class="flex flex-col md:flex-row">
                <h2 class="mb-2 text-left text-2xl font-black capitalize">{data.title}</h2>
                <span class="ml-2 px-2 border-t-2 rounded-b text-xs uppercase bg-indigo-800 w-50">{data.category}</span>
              </div>
              <p class="mt-3 font-sans text-left text-base tracking-normal">{data.description}</p>
              <div class="flex flex-col md:flex-row md:items-end">
                <p class="mt-6 text-4xl font-black">₹{data.price}<sup class="align-super text-sm">00</sup></p>
                <span class="ml-2 text-xs uppercase text-lime-400">{data.stock} In stock</span>
              </div>
            ...
            <div id="thumbid" class="pthumb zoom  mx-auto flex  px-5 pt-1 md:p-8" >
              <img id="thumbi" class=" block h-auto max-w-full rounded-md shadow-lg"
            ...
            <div className=' justify-center flex md:block md:p-8 md:px-5 pt-4 md:pt-0'>
              <span className='top  '><i class="fa fa-arrow-circle-up hover:text-indigo-500 hover:cursor-pointer animate-bounce" aria-hidden="true"></i></span>
              <div class="h-34 w-28  px-5 mt-1  md:px-0 hover:cursor-pointer
              relative  overflow-hidden bg-cover bg-no-repeat" >
                <img class="transition duration-300 ease-in-out hover:scale-110" onClick={() => ichnage(0)}
                  src={data ? (data.images ? data.images[0] : { imagenotf }) : ""} alt={data.title} />
              </div>
              <div class="h-34 w-28  px-5 mt-1  md:px-0 hover:cursor-pointer
              relative  overflow-hidden bg-cover bg-no-repeat" >
                <img class="transition duration-300 ease-in-out hover:scale-110" onClick={() => ichnage(1)}
                  src={data ? (data.images ? data.images[1] : { imagenotf }) : ""} alt={data.title} />
              </div>
              <div class="h-34 w-28  px-5 mt-1  md:px-0 hover:cursor-pointer
              relative  overflow-hidden bg-cover bg-no-repeat" >
                <img class="transition duration-300 ease-in-out hover:scale-110" onClick={() => ichnage(2)}
                  src={data ? (data.images ? data.images[2] : { imagenotf }) : ""} alt={data.title} />
              </div>
              <span className='bottom '><i class="fa fa-arrow-circle-down hover:text-indigo-500 hover:cursor-pointer animate-bounce" aria-hidden="true"></i></span>
            </div>
          </div>
        </div>
      </section>
