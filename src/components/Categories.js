...
const Categories = () => {
  ...
  return (
    ...
    {data.map((item, index)=>{
       ...
       return (
        <>
     <div class="relative  flex w-full max-w-xs flex-col overflow-hidden rounded-lg border border-gray-100 bg-white shadow-md" key={index}>
     <a class="relative flex items-center h-52 overflow-hidden" href={`/products/${item.id}`}>
     ...
     <div class="mt-4 px-5 pb-2">
       <Link to={`/products/${item.id}`}>
         <h5 class=" text-xl heading ptitle text-left tracking-tight text-slate-900">{item.title} </h5>
       </Link>
       ...
       <button onClick={() => { dispatch({ type: "ADD", payload: item }); }} class="flex w-full items-center justify-center rounded-md bg-purple-800 px-5 py-2 text-center text-sm font-medium text-white hover:bg-white hover:text-purple-800 border-2	 border-purple-700 focus:outline-none focus:ring-4 focus:ring-blue-300">
